...
def _order_paths(paths, D_groups, H_groups):
    ...
    ordered_paths = dict()
    ordered_paths['D'] = [[ordered_paths[i] for i in D_groups[group_id]] for group_id in range(len(D_groups))]
    ordered_paths['H'] = [[ordered_paths[i] for i in H_groups[group_id]] for group_id in range(len(H_groups))]
...
def aggregate_checkpoints(paths, pytorch_format=True):
    ...
    if combine_zero and combine_megatron:
        ...
        aggregate_state = _aggregate_over_ranks(ordered_paths['D'][0], mode = _AGGREGATION_MODE.Zero, pytorch_format = pytorch_format)
    elif combine_megatron:
        aggregate_state = _aggregate_over_ranks(ordered_paths['H'][0], mode = _AGGREGATION_MODE.Megatron, pytorch_format = pytorch_format)
